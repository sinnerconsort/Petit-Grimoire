 ...
 * 
 * FIXES:
 * - Duplicate DOM guard (no more stacking grimoires)
 * - Close button (×) on the book itself
 * - Compact brooch hides when grimoire opens, reappears on close
 * - z-index layering corrected
 * - Overlay click & Escape key close properly
 */
...
function createGrimoireDOM() {
    // ── Duplicate guard ──
    // If grimoire already exists, destroy it first (re-init safe)
    const existing = document.getElementById('mg-grimoire');
    if (existing) {
        existing.remove();
        console.log('[Grimoire] Removed existing grimoire DOM');
    }
    const existingOverlay = document.getElementById('mg-grimoire-overlay');
    if (existingOverlay) {
    ...
    grimoire.innerHTML = `
        <div class="mg-grimoire-book">
            ...
            <!-- Close button — sits on the book cover -->
            <button class="mg-grimoire-close" id="mg-grimoire-close" title="Close Grimoire">
...
// Track the escape handler so we can avoid stacking
...
function setupEventListeners() {
    ...
    // Close button
    const closeBtn = document.getElementById('mg-grimoire-close');
    ...
    // Escape key to close (remove old handler first to prevent stacking)
    if (_escapeHandler) {
...
// ══════════════════════════════════════════════
// COMPACT BROOCH VISIBILITY
// (The brooch "transforms into" the book — hide while open)
// ══════════════════════════════════════════════
...
function hideCompact() {
    ...
    if (compact) {
        ...
        compact.style.setProperty('transition', 'opacity 0.25s ease', 'important');
    }
}
...
function showCompact() {
    ...
    if (compact) {
        ...
        compact.style.setProperty('transition', 'opacity 0.3s ease 0.15s', 'important');
    }
}
...
export function openGrimoire() {
    ...
    const overlay = document.getElementById('mg-grimoire-overlay');
    if (!grimoire || !overlay) {
        console.warn('[Grimoire] DOM not found — reinitializing');
        createGrimoireDOM();
        setupEventListeners();
        return openGrimoire(); // retry once
    }
    
    grimoireState.isAnimating = true;
    
    // Hide the compact brooch (it "becomes" the book)
    hideCompact();
        ...
        tarot: {
            ...
            right: `
                ...
                <div class="mg-page-section">
                    ...
                    <p class="mg-text-dim">No readings yet</p>
                </div>
            `
        },
        ...
        crystal: {
            left: `
                <div class="mg-page-section">
                    ...
                    <p class="mg-page-flavor">"The crystal doesn't negotiate."</p>
            ...
            right: `
                <div class="mg-page-section">
                    ...
                    <div class="mg-vision-log" id="mg-vision-log">
                        <p class="mg-text-dim">The crystal is quiet... for now.</p>
                    </div>
                </div>
                
                <div class="mg-page-section">
                    <h4 class="mg-page-subtitle">Wild Magic</h4>
                    <p class="mg-text-dim">
        ...
        ouija: {
            left: `
                <div class="mg-page-section">
                    ...
                    <p class="mg-page-flavor">"Ask, and fate answers."</p>
            ...
            right: `
                <div class="mg-page-section">
                    <h4 class="mg-page-subtitle">Answers</h4>
                    <div class="mg-ouija-history" id="mg-ouija-history">
                        <p class="mg-text-dim">The board awaits your question.</p>
                    </div>
                </div>
                
                <div class="mg-page-section">
                    <h4 class="mg-page-subtitle">The Voice of Fate</h4>
                    <p class="mg-text-dim">
